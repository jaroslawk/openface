...
def getRep(bgrImg):
    ...
    for bb in bb:
        ...
        landmarks = align.findLandmarks(annotatedFrame, bb)
        for p in openface.AlignDlib.OUTER_EYES_AND_NOSE:
            cv2.circle(annotatedFrame, center=landmarks[p], radius=3,
                       color=(102, 204, 255), thickness=-1)

    return (reps, annotatedFrame)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--network', type=str, default='first')

    args = parser.parse_args()


    img_dim = args.imgDim
    network_model = args.networkModel

    if args.network != 'first':
        network_model = './models/openface/VGG_FACE.changed.t7'
        img_dim = 224
    ...
    net = openface.TorchNeuralNet(network_model, imgDim=img_dim, cuda=args.cuda)
    ...
    while True:
        ...
        resp, annotatedImage = getRep(frame)

        print ("P: " + str(resp))

        cv2.imshow('', annotatedImage)
